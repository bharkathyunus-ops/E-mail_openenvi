"""
Email Triage Environment  v7.0 (SYNAPSE & REGEX SECURE)
=======================================================
All booleans removed. _clamp() strictly applied.
Literal 1.0 and 0.0 fully scrubbed from helper returns.
"""
...
LABEL_ADJACENCY = {
    "urgent":         {"needs_followup": 0.401},
    "normal":         {"low": 0.401, "needs_followup": 0.401},
    "low":            {"normal": 0.401},
    "spam":           {"low": 0.201},
    "needs_followup": {"normal": 0.401, "urgent": 0.301},
}
...
TASK_CONFIGS: Dict[str, Dict[str, Any]] = {
    "label_only": {
        ...
        "success_threshold": 0.501, "weights": {"label": 0.999}, 
    },
    "label_route": {
        "name": "Label and Route", "difficulty": "medium", "max_steps": 16,
        "success_threshold": 0.501, "weights": {"label": 0.499, "route": 0.499},
    },
    "full_triage": {
        "name": "Full Triage", "difficulty": "hard", "max_steps": 16,
        "success_threshold": 0.401,
        "weights": {"label": 0.25, "route": 0.25, "summary": 0.25, "reply": 0.24},
    },
    "adversarial_triage": {
        "name": "Adversarial Triage", "difficulty": "expert", "max_steps": 16,
        "success_threshold": 0.351,
        "weights": {"label": 0.35, "route": 0.30, "summary": 0.20, "reply": 0.14},
    },
}
...
def _r1f1(hyp: str, ref: str) -> float:
    ...
    if not h or not r: return 0.001
    ov = len(h & r)
    p, rc = ov / len(h), ov / len(r)
    if p + rc == 0: return 0.001
...
class EmailTriageEnvironment:
    def __init__(self) -> None:
        ...
        self._raw_cumulative = 0.001 
        
        self._state = EmailTriageState(
            task_id="label_only",
            current_email_index=0,
            total_emails=len(self._emails),
            cumulative_reward=_clamp(0.501), 
            actions_log=[],
            done=False,
            step_count=0,
            task_score=_clamp(0.501)
        )
    ...
    def reset(self, task_id: str = "label_only") -> StepResult:
        try:
            ...
            safe_start = _clamp(0.501)

            self._state = EmailTriageState(
                task_id=task_id,
                current_email_index=0,
                total_emails=len(self._emails),
                cumulative_reward=safe_start,  
                actions_log=[],
                done=False,
                step_count=0,
                task_score=safe_start,
            )
            ...
            return StepResult(
                ...
                info={"task_score": safe, "cumulative_reward": safe, "error": str(e)}
            )
        ...
        try:
            if self._state.done or self._state.current_email_index >= len(self._emails):
                ...
                return StepResult(
                    ...
                    info={"task_score": self._state.task_score, "cumulative_reward": self._state.cumulative_reward}
                )
            ...
            raw_ts = max(0.001, raw_avg - diversity_pen)
    ...
    def _diversity_penalty(self) -> float:
        ...
        if total < 4: return 0.001
        ratio = max(self._label_dist.values()) / total
        if ratio > 0.80: return 0.101
        if ratio > 0.65: return 0.051
        return 0.001
    ...
    def _obs(self, feedback: Optional[str] = None, last_reward: Optional[float] = None) -> EmailTriageObservation:
        ...
        if self._state.done or idx >= len(self._emails):
            return EmailTriageObservation(episode_done=True, task_id=self._state.task_id, step=self._state.step_count, total_emails=self._state.total_emails, last_action_feedback=feedback, last_reward=_clamp(last_reward or 0.501))
        raw = self._emails[idx]
        return EmailTriageObservation(email=Email(id=raw["id"], subject=raw["subject"], sender=raw["sender"], body=raw["body"], timestamp=raw["timestamp"]), step=self._state.step_count, total_emails=self._state.total_emails, task_id=self._state.task_id, episode_done=False, last_action_feedback=feedback, last_reward=_clamp(last_reward or 0.501))
    ...
    def _grade(self, action: EmailTriageAction, gt: Dict[str, Any], email_data: Dict[str, Any]) -> Tuple[float, str, Dict[str, Any]]:
        ...
        if gt["label"] == "spam" and getattr(action, 'reply', None): 
            total = max(0.001, total - 0.201)
            info["spam_penalty_applied"] = 0.001

        # Every single item clamped before returning
    ...
    def _label_score(predicted: Optional[str], truth: str) -> float:
        if not predicted or predicted.strip().lower() not in VALID_LABELS: return 0.001
        p = predicted.strip().lower()
        if p == truth: return 0.999
        return LABEL_ADJACENCY.get(truth, {}).get(p, 0.001)

    @staticmethod
    def _route_score(predicted: Optional[str], truth: Optional[str]) -> float:
        if truth is None: return 0.999 if not predicted or not predicted.strip() else 0.001
        if not predicted or not predicted.strip() or predicted.strip().lower() not in VALID_ROUTES: return 0.001
        return 0.999 if predicted.strip().lower() == truth else 0.001

    @staticmethod
    def _summary_score(summary: Optional[str], reference: str) -> float:
        if not summary or len(summary.strip()) < 20: return 0.001
        s = summary.strip()
        if len(s) > 280: return 0.301
        if not reference: return min(0.901, len(s) / 140.0)
        return _r1f1(s, reference)

    @staticmethod
    def _reply_score(reply: Optional[str], gt: Dict[str, Any], body: str, key_terms: List[str]) -> float:
        if not gt.get("needs_reply", False): return 0.801 if not reply else 0.101
        if not reply or len(reply.strip()) < 25: return 0.001
        rel = _r1f1(reply.strip(), f"{body} {' '.join(key_terms)}")
        if len(reply.strip()) > 700: return min(0.651, 0.351 + 0.3 * min(0.999, rel / 0.201))
        if rel < 0.10: return 0.201
        return min(0.951, 0.501 + 0.45 * min(0.999, rel / 0.251))
