...
def _r1f1(hyp: str, ref: str) -> float:
    ...
    return 0.0 if p + rc == 0 else round(2 * p * rc / (p + rc), 4)
...
class EmailTriageEnvironment:
    ...
    def _route_score(predicted: Optional[str], truth: Optional[str]) -> float:
        if truth is None:   # spam — correct to have no route
            return 1.0 if (not predicted or not predicted.strip()) else 0.0
        if not predicted or not predicted.strip():
            return 0.0
        p = predicted.strip().lower()
        return (1.0 if p == truth else 0.0) if p in VALID_ROUTES else 0.0
    ...
    def _summary_score(summary: Optional[str], reference: str) -> float:
        ...
        if len(s) > 280:
            return 0.30
        if not reference:
            return min(0.9, len(s) / 140.0)
        return round(min(0.95, _r1f1(s, reference) / 0.35), 4)
    ...
    def _reply_score(
        ...
        if not needs:
            return 0.8 if not reply else 0.1
        ...
        rel = _r1f1(r, ref)
        if len(r) > 700:
            return round(min(0.65, 0.35 + 0.3 * min(1.0, rel / 0.20)), 4)
        if rel < 0.10:
            return 0.2
        return round(min(0.95, 0.50 + 0.45 * min(1.0, rel / 0.25)), 4)
